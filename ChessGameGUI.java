...
public class ChessGameGUI extends JFrame{
		...
		for (int row = 0; row < 8; row++){
			for (int col = 0; col < 8; col++) {
				...
				if (piece != null) {
		            ...
		            squares[row][col].setFont(new Font("Serif", Font.PLAIN, 40));
	...
	private void handleSquareClick(int row, int col) {
	    ...
	    if (selectedPiece instanceof King) {
	        System.out.println("King selected at position: (" + row + ", " + col + ")");
	    }
	    ...
	    if (moveResult) {
	    	...
	    	System.out.println("Piece selected but not moved.");
	...
	private void clearHighlights() {
		for (int row = 0; row < 8; row++) {
			for (int col = 0; col < 8; col++) {
				squares[row][col].setBackground((row + col) % 2 == 0 ? Color.LIGHT_GRAY : new Color(39, 52, 42));
		    }
		}
	}
}
